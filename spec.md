...
| Feature                            | Reason cut                                  |
| ---------------------------------- | ------------------------------------------- |
| User authentication / accounts     | Out of scope for MVP                        |
...
| User authentication / multi-tenant | Out of scope for MVP                        |
...
9. User clicks "Save Schema"
       ↓
10. Schema saved to MongoDB Atlas with precisely locked CSS selectors
        ↓
11. User exports → Downloads JSON file
    (Live API endpoint immediately callable by Agent via schema_id)
```
...
> Schemas are stored in **MongoDB Atlas** (cloud-hosted MongoDB). The backend connects via the official `mongoose` driver using a `MONGODB_URI` environment variable.

### 9.1 MongoDB Collection: `schemas`

**Connection:** `MONGODB_URI=mongodb+srv://<user>:<password>@<cluster>.mongodb.net/agenticview`

**Document structure (Mongoose schema):**

```typescript
// Collection: schemas
{
  _id: ObjectId,                     // auto-generated by MongoDB
  schema_id: string,                 // "schema_abc123" — human-readable public ID
  name: string,                      // "Amazon Product Page"
  source_url: string,                // "https://amazon.com/dp/B08N5WRWNW"
  created_at: Date,                  // 2026-02-24T10:30:00Z
  fields: [
    {
      name: string,                  // "product_name"
      css_selector: string,          // "#productTitle"
      data_type: string,             // "string" | "number" | "currency" | "date" | "boolean"
      confidence: number,            // 0.94
    }
  ],
  sample_output: Record<string, unknown>,  // { product_name: "Running Shoes Pro X", price: 129.99 }
}
```

**Indexes:**

- `schema_id`: unique index — used by the Live API Endpoint (`GET /api/v1/fetch/{schema_id}`)
...
| Technology    | Purpose                                                         |
| ------------- | --------------------------------------------------------------- |
| MongoDB Atlas | Cloud-hosted MongoDB — stores schemas and fields via `mongoose` |
| `mongoose`    | ODM for schema modeling, validation, and connection pooling     |
...
| Storage      | MongoDB Atlas (via `mongoose`)                          | MongoDB Atlas + user accounts + rate limiting                                                     |
| API Endpoint | `GET /api/v1/fetch/{schema_id}` backed by MongoDB Atlas | Same endpoint + auth + per-user schema isolation                                                  |
...
- Connect MongoDB Atlas: configure `MONGODB_URI`, set up `mongoose` connection and `Schema` model.
...
- Implement `GET /api/v1/fetch/{schema_id}` — read schema from MongoDB Atlas, run Playwright, return structured JSON.
